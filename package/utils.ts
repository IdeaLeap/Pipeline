 ...
 * ```
 */
export function replaceSlots(obj: Record<string, any>, replacements: Record<string, any>): any {
  return lodash.cloneDeepWith(obj, (value: any) => {
    if (lodash.isString(value)) {
      ...
      if (fullMatchReplacement && !!fullMatchReplacement[1]) {
        const replacementValue = lodash.get(replacements, fullMatchReplacement[1]);
 ...
 */
export function isRecordOfString(variable: any): variable is Record<string, string> {
  return lodash.isPlainObject(variable) && lodash.every(variable, lodash.isString);
}